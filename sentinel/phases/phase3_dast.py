...
def propose_and_run(
    ...
    """One full Phase 3 cycle: propose -> approve (or refuse) -> execute -> report.

    Returns None if the proposal was blocked (scope/destructive/engagement-type)
    or denied by the human — callers should treat both as "did not run", the
    distinction is in the log. Raises the approval module's exceptions upward
    for blocked cases so the caller can't accidentally swallow them.
    """
    ...
    record = approval.gate(proposal, engagement.scope, engagement.logger, confirm_fn=confirm_fn)
